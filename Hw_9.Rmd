...
## Part B
This statement claims that the evidence suggests that the minority percentage and the age of the houses are both related to the number of FAIR policies in a ZIP code, and is true. This can be seen in Figure B1, where the housing age and the minority percentage of the community is compared. It shows a moderate positive linear relationship between the two, indicating that they are related and since we have established that the minority percentange is positively related with the number of FAIR policies passed, all three of them can be said to have positvely related interactions.
...
This statement is true. The minority:fire_riskLow represents the effect of both being in a minority ZIP code and having a low fire risk, and since the variable for being in a minority ZIP code and having a high fire risk is not there we can assume it is the intercept. Since the 

## Part D
Even without controlling for any other variables, income “explains away” all the association between
minority percentage and FAIR policy uptake.



## Part E
Minority percentage and number of FAIR policies are still associated at the ZIP code level, even after
...
This statement is true. We can see from the first plot (A1) that there
is a positive correlation between residents who are minorities and
residents using FAIR policies. The confidence interval for the slope is
[0.009, 0.018]. Since the 95% confidence interval does not include 0,
and the p-value is near 0, there is statistically significant evidence
to conclude that ZIP codes with a higher percentage of minority
residents tend to have more FAIR policies per 100 housing units.

**B. The evidence suggests an interaction effect between minority
percentage and the age of the housing stock in the way that these two
variables are related to the number of FAIR policies in a ZIP code.**

This is false. There is no model showing a joint interaction between
minority percentage and age of the housing stock.

**C. The relationship between minority percentage and number of FAIR
policies per 100 housing units is stronger in high-fire-risk ZIP codes
than in low-fire-risk ZIP codes.**

This is false. We can see in the interaction for minorities with
high-fire risk that the p-value is 0.839, which means it is not
statistically significant. We cannot say that the slopes are that
different. The p-value for minorities in the low-risk-fire group is
0.035. This is statistically significant.

**D. Even without controlling for any other variables, income “explains
away” all the association between minority percentage and FAIR policy
uptake.**

This is false. Adding income into the regression reduces the coefficient
slightly (from 0.014 to 0.01).

**E. Minority percentage and number of FAIR policies are still
associated at the ZIP code level, even after controlling for income,
fire risk, and housing age.**

This is true. We can see from the regression line that adjusts for all
of these variables that the p-value is 0.006, so that means it is
statistically significant. The coefficient for minority (0.008) is still
positive.
